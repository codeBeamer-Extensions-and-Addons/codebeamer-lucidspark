...
export class ImportModal extends LucidCardIntegrationCustomImportModal {
	...
	private getCardBlocks(): void {
		const itemsInCollection = this.data.dataSources
			.find((source) => source.getName() === DataConnectorName)
			?.collections.find(
				(collection) => collection.getName() === 'items'
			)?.items;
		...
		const itemsOnBoard =
			itemsInCollection?.filter((itemInCollection) =>
				cardBlocks.some(
					(cardBlock) =>
						(
							cardBlock.properties.get('RK_1') as {
								cid: string;
								pk: string;
							}
						).pk === itemInCollection.primaryKey
				)
			) || [];
		...
		const data = itemsOnBoard.map((item) => ({
			cardBlockId: cardBlocks.find(
				(cardBlock) =>
					(
						cardBlock.properties.get('RK_1') as {
							cid: string;
			...
			codebeamerItemId: item.fields.get('Id'),
			codebeamerTrackerId: item.fields.get('Tracker Id'),
		}));
	...
	private getLines(): void {
		const lines = this.viewport
			.getCurrentPage()
			?.allLines.filter((line) => line instanceof LineProxy) as LineProxy[];
		...
		const data = linesConnectedToCardBlocks.map((line) => ({
			...
			targetBlockId: (line.getDownstreamConnection() as CardBlockProxy).id,
		}));
	...
	/**
	 * Retrieves and sends the OAuth token to the modal.
	 */
	private async getOAuthToken(): Promise<void> {
		const token = await this.client.getOAuthToken('google');
		const message = { payload: [token], action: 'getOAuthToken' };
		this.sendMessage(JSON.stringify(message));
	}
	 ...
	 */
	private deleteLine(importId: number, lineId: string): void {
		const line = this.lines.find((line) => line.id === lineId);
		if (line) line.delete();

		this.imports.get(importId)!.totalItems--;
		if (this.imports.get(importId)!.totalItems <= 0) {
