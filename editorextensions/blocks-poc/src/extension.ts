import {CardBlockProxy, EditorClient, Menu, MenuLocation, MenuType, PageProxy, SimpleImageFillPosition, StatusValues, Viewport} from 'lucid-extension-sdk';
import {ImportModal} from './importmodal';

const client = new EditorClient();
const menu = new Menu(client);
const viewport = new Viewport(client);

client.registerAction('test', () => {
    const modal = new ImportModal(client);
    modal.show();
});

client.registerAction('create-sticky', async () => {
    if(viewport.getCurrentPage() != null) {
        createSticky(viewport.getCurrentPage()!, viewport.getVisibleRect().x, viewport.getVisibleRect().y);
    } else {
        console.log('No page selected');
    }
});

menu.addMenuItem({
    label: 'Test thing 2',
    action: 'test',
    menuType: MenuType.Main,
    location: MenuLocation.Extension
});

// add menu item for create-sticky action
menu.addMenuItem({
    label: 'Create sticky',
    action: 'create-sticky',
    menuType: MenuType.Main,
    location: MenuLocation.Extension
});

async function createSticky(page: PageProxy, x: number, y: number) {
    await client.loadBlockClasses(['LucidCardBlock']);
    const block = page.addBlock({
        className: 'LucidCardBlock',
        boundingBox: {
            x, y, w:420, h:160
        }
    })

    if (block instanceof CardBlockProxy) {
        block.setTitle('Generated card');
        block.setDescription('This card was generated by an extension');
        block.setAssignee('Ethan');
        block.setEstimate(3);
        block.setStatus(StatusValues.Todo);
        block.setFillStyle({position: SimpleImageFillPosition.Fit, url: icon});
    }
}

const icon = 'data:image/png;base64,';

